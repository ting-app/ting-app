              ...
              <td class="text-center">
                <v-menu offset-y>
                  ...
                  <v-list>
                    ...
                    <v-list-item class="pointer" @click="deleteProgram(index, program)"
                    >
...
export default {
  ...
  methods: {
    ...
    deleteProgram (i, program) {
      if (!window.confirm(`确认删除${program.title}？`)) {
        return
      }

      this.loading = true

      axios.delete(`/programs/${program.id}`)
        .then((response) => {
